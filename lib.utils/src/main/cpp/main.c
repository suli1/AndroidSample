...
#define TARGET_CRYPT_SIG "([B[BJI)[B"
...
JNIEXPORT jbyteArray JNICALL android_native_aes(JNIEnv *env, jclass clazz,
                                                jbyteArray jarray, jbyteArray key, jlong jtimestamp,
                                                jint jmode) {
    //check input data
    ...
    uint8_t *data = (uint8_t *) (*env)->GetByteArrayElements(env, jarray, NULL);
    if (!data) {
        return NULL;
    }

    uint8_t *AES_KEY = (uint8_t *) (*env)->GetByteArrayElements(env, key, NULL);
    jsize keyLen = (*env)->GetArrayLength(env, key);
    if (!AES_KEY || keyLen != 16) {
        return NULL;
    }

    uint8_t *AES_IV = AES_KEY;
    ...
    //设置输入
    uint8_t *input = (uint8_t *) malloc(src_len);
    memset(input, 0, src_len);
    memcpy(input, data, len);
    if (padding_len > 0) {
        memset(input + len, (uint8_t) padding_len, padding_len);
    }
    //data不再使用
    (*env)->ReleaseByteArrayElements(env, jarray, data, 0);

    //设置输出Buffer
    uint8_t *buff = (uint8_t *) malloc(src_len);
    if (!buff) {
        free(input);
        return NULL;
    }
